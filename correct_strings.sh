...
find "$mydir/vector/src/main/res" -name strings.xml -exec \
    sed -i 's|\("use_latest_riot">.*\)LeOS-Matrix\(.*</string>\)|\1Element\2|g' '{}' \;
find "$mydir/vector/src/main/res" -name strings.xml -exec \
    sed -i 's|\("use_other_session_content_description">.*\)LeOS-Matrix\(.*LeOS-Matrix.*</string>\)|\1LeOS-Matrix/Element\2|' '{}' \;
...
git add -A
git commit -m "Automatic LeOS-Matrix string correction"
